...
import { ListField } from "lib/uniforms-surfnet/src/index";
...
describe("<VlanField>", () => {
    ...
    test("<VlanField> - renders an input with correct placeholder (port chosen)", async () => {
        mock.onGet("subscriptions/vlans-by-service-port/abc?nsi_vlans_only=false").reply(200, []);
    ...
    test("<VlanField> - changes value based on portMode (untagged)", async () => {
        mock.onGet("surf/subscriptions/vlans-by-service-port/abc?nsi_vlans_only=false").reply(200, []);
    ...
    test("<VlanField> - renders an input that is correctly enabled/disabled based on portMode (untagged)", async () => {
        ...
        // expect(fetchMock).toHaveFetchedTimes(1, "glob:*/api/surf/subscriptions/vlans-by-service-port/abc");
    ...
    test("<VlanField> - renders an input that is correctly enabled/disabled based on portMode (untagged with used vlans)", async () => {
        mock.onGet("surf/subscriptions/vlans-by-service-port/abc?nsi_vlans_only=false").reply(200, [[0, 0]]);
    ...
    test("<VlanField> - adds vlans taken in same list as used vlans", async () => {
        mock.onGet("surf/subscriptions/vlans-by-service-port/abc?nsi_vlans_only=false").reply(200, []);
        mock.onGet("surf/subscriptions/vlans-by-service-port/def?nsi_vlans_only=false").reply(200, []);
        ...
        expect(wrapper.find(".euiFormRow__text").at(0).props("children")["children"]).toBe(
            "Already used VLAN ranges for this service port: 3"
        );
        ...
        // expect(fetchMock).toHaveFetchedTimes(2, "glob:*/api/surf/subscriptions/vlans-by-service-port/abc");
        // expect(fetchMock).toHaveFetchedTimes(1, "glob:*/api/surf/subscriptions/vlans-by-service-port/def");
        expect(wrapper.render()).toMatchSnapshot();
    });
    ...
    test("<VlanField> - renders extra errors correctly (tagged and taken)", async () => {
        mock.onGet("surf/subscriptions/vlans-by-service-port/abc?nsi_vlans_only=false").reply(200, [[2, 2]]);
    ...
    test("<VlanField> - renders extra errors correctly (ims error)", async () => {
        mock.onGet("surf/subscriptions/vlans-by-service-port/abc?nsi_vlans_only=false").reply(400, []);
