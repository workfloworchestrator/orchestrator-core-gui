...
export default class TopologyDiagram extends React.Component<IProps, IState> {
            ...
            if (endpoint.type === "port") {
                ...
                return this.context.customApiClient.serviceByImsServiceId(endpoint.id).then((result: IMSService) => {
                    if (["SP", "MSP", "SSP"].includes(result.product)) {
                        ...
                        const internalPortId = result.endpoints.find((e) => e.type === "internal_port");
                        if (internalPortId) {
                            return this.context.customApiClient
                                .internalPortByImsPortId(internalPortId.id)
                                .then((port: any) =>
                                    Object.assign(port, {
                                        serviceId: endpoint.id,
                                    })
    ...
    _makeConnectionExplanation = (endpoint: IMSEndpoint, sap: any, sub: SubscriptionModel): JSX.Element => {
        ...
        const portmode = block && block.port_mode ? block.port_mode : "N/A";
    ...
    _buildIP(): Topology {
        ...
        if (subscription.vc.saps) {
            ...
            subscription.vc.saps.forEach((sap: any, index: number) => {
                ...
                const label = endpoint?.port
                    ? `${endpoint?.node}__${endpoint?.port.replace(/\//g, "_")}`
                    : endpoint?.name;
            ...
            console.log("Problem with resolving/handling the endpoints");
        }
