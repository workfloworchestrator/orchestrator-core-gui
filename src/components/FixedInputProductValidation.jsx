...
import "./FixedInputProductValidation.css";
    ...
    render() {
        ...
        return (
            <section className="fixed-input-validation">
                <h3 onClick={() => history.push("/product/" + validation.id)}
                    className="description">{I18n.t("validations.fixedInput.title", {name: validation.name})}</h3>
                <table>
                    <thead>
                    <tr>
                    ...
                    <tbody>
                    {
                        validation.errors.map((error, index) => <tr key={index}>
                            <td>{error.name}</td>
                            <td>{I18n.t(`validations.fixedInput.error.${error.error}`, {value: error.value})}</td>
                        </tr>)
                    }
                    </tbody>
                </table>
