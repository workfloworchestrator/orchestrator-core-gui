...
export default class WorkFlows extends React.Component {
    ...
    componentDidMount() {
        allWorkflowsWithProductTags().then(res => {
            ...
            uniqueWorkflowTags.forEach(tag => newFilterAttributesProductTag.push({
                name: tag,
                selected: true,
            ...
            uniqueTargets.forEach(target => newFilterAttributesTarget.push({
                name: target,
                selected: true,
            ...
            this.setState({
                workflows: res, filteredWorkflows: res,
    ...
    delayedSearch = debounce(query => {
        ...
        this.setState({
            query: query,
            filteredWorkflows: this.doSearchAndSort(query, workflows, sorted, filterAttributesProductTag, filterAttributesTarget)
        });
    }, 250);
    ...
    filter = name => item => {
        ...
        this.setState({
            ...
            filteredWorkflows: this.doSearchAndSort(query, workflows, sorted, newFilterAttributesProductTag, newFilterAttributesTarget)
        });
    };
    ...
    renderWorkflows(workflows, actions, sorted) {
        ...
        const td = (name, workflow) => <td key={name} data-label={I18n.t(`metadata.workflows.${name}`)} className={name}>
            {workflow[name]}
        </td>;
                ...
                <table className="workflows">
                    ...
                    {workflows.map((workflow, index) =>
                        <tr className={this.workFlowClassName(workflow)} key={`${workflow.workflow_id}_${index}`}>
                            ...
                            <td data-label={I18n.t("metadata.workflows.created_at")}
                    ...
                    <tr>
                        <td className="metadata-results" colSpan={6}>{I18n.t("metadata.results", {type: "Workflows", count: workflows.length})}</td>
                    </tr>
                    </tbody>
                </table>
            );
        }
            return <div><em>{I18n.t("metadata.workflows.no_found")}</em></div>;
    }
    workFlowClassName = workflow => workflow.target !== "SYSTEM" && workflow.product_tags.length === 0 ? "invalid" : "";

    render() {
        const {
            filteredWorkflows, actions, query, sorted, filterAttributesProductTag, filterAttributesTarget
        } = this.state;
        return (
            <div className="mod-workflows">
                <div className="options">
                    <FilterDropDown items={filterAttributesProductTag}
                                    filterBy={this.filter("workflow_tag")}
                                    label={I18n.t("metadata.workflows.tag")}/>
                    <FilterDropDown items={filterAttributesTarget}
