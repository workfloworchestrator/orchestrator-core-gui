...
export default class OldSubscriptions extends React.PureComponent {
    constructor(props) {
        super(props);
    ...
    componentDidMount = () =>
        allSubscriptions().then(results => {
            ...
            newFilterAttributesStatus.forEach(
                attr => (attr.count = results.filter(sub => sub.status === attr.name).length)
            );
            ...
            this.setState({
                ...
                filteredSubscriptions: this.doSearchAndSortAndFilter(
                    "",
                    results,
                    this.state.sorted,
                    filterAttributesProduct,
    ...
    doSearchAndSortAndFilter = (query, subscriptions, sorted, filterAttributesProduct, filterAttributesStatus) => {
        if (!isEmpty(query)) {
            ...
            if (colonIndex > -1) {
                    ...
                    subscriptions = subscriptions.filter(subscription => {
                        ...
                        if (key === "global_search") {
                            const filteredColumns = searchableColumns.filter(
                                col => !Object.keys(searchOptions).includes(col)
                            );
                ...
                subscriptions = subscriptions.filter(subscription =>
                    searchableColumns
                        .map(search => subscription[search].toLowerCase().indexOf(queryToLower))
                        .some(indexOf => indexOf > -1)
                );
            }
        }
    ...
    renderSubscriptionsTable(filteredSubscriptions, sorted, collapsibleSubscriptions, collapsedSubscriptions) {
        ...
        return (
            <div>
                <em>{I18n.t("subscriptions.no_found")}</em>
            </div>
        );
    }
    ...
    renderFetchingSpinner = () => (
        <section className="fetching-related-subscriptions">
            <i className="fa fa-refresh fa-spin fa-2x fa-fw" />
            <em>{I18n.t("subscriptions.fetchingRelatedSubscriptions")}</em>
        </section>
    );

    relatedSubscriptionMsg = subscription =>
        ["LightPath", "LPNLNSI", "ELAN"].includes(subscription.tag)
            ? I18n.t("subscriptions.relatedSubscriptionsLP", {
                  description: subscription.description
              })
            : I18n.t("subscriptions.relatedSubscriptionsServicePort", {
                  description: subscription.description
              });
    ...
    renderSubscriptionRow = (subscription, index, collapsibleSubscriptions, collapsedSubscriptions) => {
        ...
        return (
            <tbody key={`${subscriptionId}_${index}`}>
                ...
                {isLoading && (
                    <tr>
                        <td colSpan="9">{this.renderFetchingSpinner()}</td>
                    </tr>
                )}
                {hasNoRelatedSubscriptions && (
                    <tr className="related-subscription">
                        ...
                        <td colSpan="9">
                            <em>
                                {I18n.t("subscriptions.noRelatedSubscriptions", {
                                    description: subscription.description
                                })}
                            </em>
                        </td>
                ...
                {renderRelatedSubscriptions && (
                    ...
                    >
                        ...
                        <td colSpan="9">
                            <em>
                                {subscription.status !== "terminated"
                                    ? this.relatedSubscriptionMsg(subscription)
                                    : I18n.t("subscriptions.terminatedWarning")}
                            </em>
                        </td>
                    </tr>
                )}
    ...
    renderSingleSubscription = (subscription, isCollapsible, icon, className = "") => {
        ...
        return (
            <tr key={subscriptionId} className={className} onClick={this.showSubscription(subscription)}>
                <td data-label={I18n.t("subscriptions.noop")} className="expand">
                    <span>
                        {isCollapsible && (
                            <i
                                className={`fa fa-${icon}-circle`}
                                onClick={this.handleCollapseSubscription(subscription)}
                            />
                        )}
                    </span>
                </td>
