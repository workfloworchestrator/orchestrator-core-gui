...
<Story name="ImsPortIdFieldCorelink">
    {() => {
        mock.reset();
        mock.onGet(/ims\/nodes\/.*\/IS/).reply(200, []);
        mock.onGet("v2/subscriptions?filter=tags%2CNode%2Cstatuses%2Cactive").reply(200, [
            {
        ...
        mock.onGet(/ims\/free_corelink_ports\/.*\/1000/).reply(200, [
        ...
        mock.onGet(/ims\/free_ports\/.*/).reply(200, [
            { id: 1, iface_type: "1000BASE-LX", port: "0/0/0", status: "IS" },
            { id: 2, iface_type: "1000BASE-LX", port: "0/0/1", status: "IS" },
            { id: 3, iface_type: "1000BASE-SX", port: "0/0/2", status: "IS" }
        ]);
...
<Story name="ImsPortIdFieldLegacy">
    {() => {
        ...
        mock.onGet("ims/nodes/MT001A/IS").reply(200, [
            {
                id: 1,
                name: "MT001A-JNX-01",
                status: "active"
